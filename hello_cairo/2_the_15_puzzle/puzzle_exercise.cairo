# https://www.cairo-lang.org/docs/hello_cairo/puzzle.html#exercise
...
func main():
    ...
    local loc_tuple : (
        ...
        # First location cases
        # Location(row=10, col=2), # Fails: 0 != 5040. assert row * (row - 1) * (row - 2) * (row - 3) = 0
        # Location(row=1, col=2),  # Fails: 1 != 0. assert col_diff * col_diff = 1
        Location(row=0, col=2),    # Succeeds
